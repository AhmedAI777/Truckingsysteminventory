...
# requirements: streamlit gspread gspread-dataframe extra-streamlit-components pandas google-auth google-auth-oauthlib pydrive2
...
def _inject_font_css(font_path: str, family: str = "ACBrandFont"):
    ...
    st.markdown(
        ...
        <style>
          ...
          html, body, [class*="css"] {{ font-family: '{family}', -apple-system, BlinkMacSystemFont, "Segoe UI",
...
# =============================================================================
# GOOGLE SHEETS (OAuth user) + DRIVE (OAuth via PyDrive2)
# =============================================================================
SCOPES = ["https://www.googleapis.com/auth/spreadsheets",
          "https://www.googleapis.com/auth/drive"]

# ---- OAuth creds (NOT cached; contains widgets during first auth) ----
def _get_user_creds():
    """OAuth creds for Sheets/Drive using client_id/secret from st.secrets (no client_secrets.json)."""
    ...
    if not creds or not creds.valid:
        ...
        if not creds or not creds.valid:
            ...
            # Try local server first (dev)
            try:
                creds = flow.run_local_server(port=0, open_browser=True)
            except Exception:
                # Headless fallback: user pastes authorization code
