...
COUNTER_WS = "Counters"  # order number tracking
...
# =========================
# Cookie manager & session defaults
# =========================
...
def upload_pdf_and_get_link(file_bytes: bytes, *, name: str, office: str, project_location: str) -> tuple[str, str]:
    """Uploads a PDF (bytes) to the Shared Drive under office/project and returns (link, file_id)."""
    SHARED_DRIVE_ID = st.secrets.get("drive", {}).get("shared_drive_id", "1KatH0TQregGV_pajnySOGcPAXTNhex7L")
    ROOT_OFFICE_FOLDER_ID = {
        # Map office label to folder id in the shared drive
        "Head Office (HO)": st.secrets.get("drive", {}).get("office_root_id", "1KatH0TQregGV_pajnySOGcPAXTNhex7L"),
    }
    ...
    service = build("drive", "v3", credentials=Credentials.from_service_account_info(_load_sa_info(), scopes=["https://www.googleapis.com/auth/drive"]))

    # Ensure project subfolder
    query = (
        f"'{office_folder_id}' in parents and name='{project_location}' and "
...
def read_worksheet(ws_title):
    try:
        ...
        elif ws_title == COUNTER_WS:
            return pd.DataFrame(columns=COUNTER_COLS)
        else:
            return pd.DataFrame()
...
def _registration_field_map() -> dict[str, str]:
    ...
    fm.update({"eq_type": fm["eq1_type"], "eq_brand": fm["eq1_brand"], "eq_model": fm["eq1_model"], "eq_specs": fm["eq1_specs"], "eq_serial": fm["eq1_serial"]})
...
def resolve_employee_fields(emp_df: pd.DataFrame, name: str) -> dict:
    ...
    if r is None:
        return {"name": name, "email": "", "mobile": "", "dept": "", "location": "", "office": ""}
    return {
...
def build_transfer_pdf_values(row: dict, new_owner: str, emp_df: pd.DataFrame) -> dict[str, str]:
    ...
    return {
        fm["from_name"]: from_fields.get("name", ""),
        fm["from_mobile"]: row.get("Contact Number", "") or from_fields.get("mobile", ""),
        fm["from_email"]: row.get("Email Address", "") or from_fields.get("email", ""),
        fm["from_department"]: row.get("Department", "") or from_fields.get("dept", ""),
        fm["from_date"]: now_str,
        fm["from_location"]: row.get("Location", "") or from_fields.get("location", ""),
...
# =========================
# Approvals state writeback & counters
# =========================
...
def _mark_decision(ws_name: str, row: dict, *, status: str):
    ...
    if not idxs:
        ...
        return
    idx = idxs[0]
    for col in ["Approval Status", "Approver", "Decision at"]:
...
def get_next_order_number(action_type: str, serial: str) -> str:
    ...
    try:
        ...
        df = pd.DataFrame(columns=COUNTER_COLS)
    for c in COUNTER_COLS:
        if c not in df.columns:
            df[c] = ""
    serial_norm = normalize_serial(serial)
    mask = (df["Action"].astype(str).str.upper() == action_type.upper()) & (df["Serial Number"].astype(str).map(normalize_serial) == serial_norm)
    if df.empty or not mask.any():
        next_no = 1
    else:
        next_no = pd.to_numeric(df.loc[mask, "Order Number"], errors="coerce").fillna(0).astype(int).max() + 1
    ...
    # append
    if df.empty:
        out = new_row[COUNTER_COLS]
    else:
        out = pd.concat([df, new_row[COUNTER_COLS]], ignore_index=True)
    ws.clear()
    set_with_dataframe(ws, out)
    st.cache_data.clear()
...
def register_device_tab():
    ...
    if download_btn:
        ...
        if not serial or not device:
            ...
            if tpl_bytes:
                emp_row = _find_emp_row_by_name(emp_df, row["Current user"]) or {}
                ...
                st.download_button(
                    ...
                    data=filled,
                    file_name=_ict_filename(serial, order_no, emp_row if isinstance(emp_row, dict) else emp_row.to_dict()),
    ...
    if submitted:
        ...
        link, fid = upload_pdf_and_get_link(
            ...
            project_location=emp_row.get("Location (KSA)", "JEDDAH (JEDDAH)") if isinstance(emp_row, dict) else "JEDDAH (JEDDAH)",
        )
        if not fid:
...
def transfer_tab():
    ...
    if dl:
        if not serial or not new_owner:
            ...
            mapped_vals = build_transfer_pdf_values(row, new_owner, emp_df)
            ...
            if tpl_bytes:
                ...
                emp_row = _find_emp_row_by_name(emp_df, new_owner) or {}
                st.download_button(
                    "📥 Download Prefilled Transfer PDF",
                    data=pdf_bytes,
                    file_name=_transfer_filename(serial, order_no, emp_row if isinstance(emp_row, dict) else emp_row.to_dict()),
                    mime="application/pdf",
                )
...
def _approve_device_row(row: pd.Series):
    ...
    emp_row_series = _find_emp_row_by_name(emp_df, current_owner)
    emp_row = emp_row_series.to_dict() if isinstance(emp_row_series, pd.Series) else {}
    order_no = get_next_order_number("REG", serial)
...
def _approve_transfer_row(row: pd.Series):
    ...
    emp_df = read_worksheet(EMPLOYEE_WS)
    emp_row_series = _find_emp_row_by_name(emp_df, new_user)
    emp_row = emp_row_series.to_dict() if isinstance(emp_row_series, pd.Series) else {}
    ...
    def _val(*cols: str) -> str:
        return _get_emp_value(emp_row_series, *cols) if isinstance(emp_row_series, pd.Series) else ""
...
def _reject_row(ws_title: str, row: pd.Series):
    ...
    try:
        ...
        if action == "Register":
            emp_df = read_worksheet(EMPLOYEE_WS)
            emp_row_series = _find_emp_row_by_name(emp_df, row.get("Current user", ""))
            emp_row = emp_row_series.to_dict() if isinstance(emp_row_series, pd.Series) else {}
            project = emp_row.get("Project", "HO")
            location = emp_row.get("Location (KSA)", "JED")
        else:
...
def export_tab():
    ...
    with pd.ExcelWriter(excel_buf, engine="xlsxwriter") as writer:
        ...
        for i, col in enumerate(df.columns):
            try:
                col_width = max(15, int(df[col].astype(str).str.len().max()))
            except Exception:
                col_width = 20
