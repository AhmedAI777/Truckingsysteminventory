...
def _diag_key():
    ...
    if pk:
        st.write("Starts with:", pk[:30].replace("\n", "\\n"))
        st.write("Ends with:", pk[-30:].replace("\n", "\\n"))
        st.write("Contains header:", "-----BEGIN PRIVATE KEY-----" in pk)
        st.write("Contains footer:", "-----END PRIVATE KEY-----" in pk)
        st.write("Newlines count:", pk.count("\n"))

with st.expander("SA key diagnostics (temporary)"):
