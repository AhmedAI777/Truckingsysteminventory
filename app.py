...
# -----------------------------
# READ SECRETS (public mode)
#   In .streamlit/secrets.toml:
#   [connections.gsheets]
#   type = "public"
#   spreadsheet = "https://docs.google.com/spreadsheets/d/<ID>/edit"
#   inventory_tab   = "0"          # gid of 'truckinventory'
#   transferlog_tab = "405007082"  # gid of 'transfer_log'
# -----------------------------
SPREADSHEET = (
    st.secrets.get("connections", {})
      .get("gsheets", {})
...
# -----------------------------
def _ws_arg(x):
    """Accept gid (as str/int) or tab name; public mode prefers gid."""
    try:
...
def load_ws(worksheet, cols):
    """Safe read (no red exceptions in UI)."""
    try:
        df = conn.read(
            spreadsheet=SPREADSHEET,
            worksheet=_ws_arg(worksheet),
            ttl=0
        )
        return _ensure_cols(df, cols)
    except Exception as e:
...
# -----------------------------
# VIEW INVENTORY (single full table)
# -----------------------------
st.subheader("Current Inventory")
...
# -----------------------------
# TRANSFER LOG (ordered + numbered)
# -----------------------------
st.subheader("Transfer Log")
log = load_ws(TRANSFERLOG_WS, LOG_COLS)

# Proper ordering by Date issued (newest first) and tidy index
if not log.empty and "Date issued" in log.columns:
...
# Add a visible row-number column starting at 1
if not log.empty:
