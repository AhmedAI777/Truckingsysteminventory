...
# # Read connection config from secrets (safe to also pass into .read/.update)
# GS = st.secrets.get("connections", {}).get("gsheets", {})
# SPREADSHEET = GS.get("spreadsheet", "").strip()  # can be ID or clean URL

# # Worksheet names (secrets override; sane defaults below)
# INVENTORY_WS   = (st.secrets.get("inventory_tab", "truckinventory") or "").strip()
# TRANSFERLOG_WS = (st.secrets.get("transferlog_tab", "transfer_log") or "").strip()  # <-- underscore default
...
# def _probe_ws(name: str) -> bool:
#     try:
#         conn.read(spreadsheet=SPREADSHEET, worksheet=name, nrows=1, ttl=0)
#         return True
#     except Exception:
#         return False

# # Light autodetect for common tab-name variants (helps when rename/new sheet)
# def _resolve_ws(preferred: str, fallbacks: list[str]) -> str:
#     for n in [preferred] + fallbacks:
#         if n and _probe_ws(n):
#             return n
#     return preferred  # will fail later with a friendly message

# INVENTORY_WS   = _resolve_ws(INVENTORY_WS,   ["df_inventory", "Inventory", "inventory"])
# TRANSFERLOG_WS = _resolve_ws(TRANSFERLOG_WS, ["transferlog", "Transfer Log", "transfer-log"])
...
url = "https://docs.google.com/spreadsheets/d/1SHp6gOW4ltsyOT41rwo85e_LELrHkwSwKN33K6XNHFI/edit?usp=sharing"
