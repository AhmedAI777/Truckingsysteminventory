...
# Pending approvals sheets (Drive removed: no file links)
APPROVAL_META_COLS = [
    "Approval Status",              # Pending / Approved / Rejected
    "Submitted by",
    "Submitted at",
...
# Employees sheet columns (canonical)
EMPLOYEE_CANON_COLS = [
    "New Employeer","Employee ID","New Signature","Name","Address",
    "Active","Position","Department","Location (KSA)",
    "Project","Microsoft Teams","Mobile Number"
...
def register_device_tab():
    ...
    with st.form("register_device", clear_on_submit=True):
        ...
        # Drive removed: no approval PDF upload required
        submitted = st.form_submit_button("Save Device", type="primary")
...
def approvals_tab():
    ...
    # Devices
    st.markdown("### Pending Device Registrations")
    df_dev = pending_dev[pending_dev["Approval Status"].isin(["", "Pending"])].reset_index(drop=True) if not pending_dev.empty else pd.DataFrame()
    if df_dev.empty:
        st.success("No pending device registrations.")
    else:
        for i, row in df_dev.iterrows():
    ...
    # Transfers
    st.markdown("### Pending Transfers")
    df_tr = pending_tr[pending_tr["Approval Status"].isin(["", "Pending"])].reset_index(drop=True) if not pending_tr.empty else pd.DataFrame()
    if df_tr.empty:
        st.success("No pending transfers.")
    else:
        for i, row in df_tr.iterrows():
...
def _mark_decision(ws_title: str, row: pd.Series, *, status: str):
    ...
    if not df.empty:
        ...
        if not mask.any():
            if "Serial Number" in df.columns:
                mask = df["Serial Number"].astype(str) == str(row.get("Serial Number", ""))
        idxs = df[mask].index.tolist() if not df.empty else []
        if not idxs:
            return
        idx = idxs[0]
        df.loc[idx, "Approval Status"] = status
