...
#     office_code = "HO"  # or derive from Office field if needed
#     city_code = "JED"   # this will be passed in later
#     order = get_next_order_number("Register", serial)
#     return f"{office_code}-{city_code}-REG-{normalize_serial(serial)}-{order}-{datetime.now().strftime('%Y%m%d')}.pdf"


# def _transfer_filename(serial: str) -> str:
#     office_code = "HO"
#     city_code = "JED"
#     order = get_next_order_number("Transfer", serial)
#     return f"{office_code}-{city_code}-TRN-{normalize_serial(serial)}-{order}-{datetime.now().strftime('%Y%m%d')}.pdf"
...
def transfer_tab():
        ...
        if not serial or not new_owner:
            ...
            if not tpl_bytes:
                ...
                st.download_button(
                    "📥 Download Prefilled Transfer PDF",
                    data=pdf_bytes,
                    file_name=f"transfer_{normalize_serial(serial)}.pdf",
                    mime="application/pdf",
                )
    ...
    if submitted:
        ...
        pending = {
            **row,
            "From owner": row.get("Current user", ""),  # actual current owner
            "To owner": new_owner,
            "Approval Status": "Pending",
            "Approval PDF": link,
            "Approval File ID": fid,
...
def approvals_tab():
    ...
    def _has_pending(df: pd.DataFrame) -> bool:
        return (not df.empty) and ("Approval Status" in df.columns) and (df["Approval Status"] == "Pending").any()

    if not _has_pending(pend_df):
        st.info("No pending device registrations.")
    else:
        for i, row in pend_df.iterrows():
            ...
            header = f"Serial: {row.get('Serial Number','—')} — {row.get('Device Type','—')}"
            with st.expander(header):
                st.write({k: ("" if pd.isna(v) else v) for k, v in row.items()})
                pdf_link = str(row.get("Approval PDF", "") or "")
                if pdf_link:
                    st.markdown(f"[📄 View PDF]({pdf_link})")
                else:
    ...
    if not _has_pending(pend_trf):
        ...
        for i, row in pend_trf.iterrows():
            ...
            hdr = f"Serial: {row.get('Serial Number','—')} — Transfer to {row.get('To owner','—')}"
...
def run_app():
    ...
    if st.session_state.get("role") == "Admin":
        ...
        with tabs[1]:
            employees_view_tab()
        with tabs[2]:
            register_device_tab()   # uses store_pdf_in_drive
        with tabs[3]:
            inventory_tab()
        with tabs[4]:
            transfer_tab()          # uses store_pdf_in_drive
        with tabs[5]:
            history_tab()
        with tabs[6]:
            approvals_tab()         # uses move_pdf_in_drive
        with tabs[7]:
            export_tab()
    else:
        tabs = st.tabs(["📝 Register Device", "🔁 Transfer Device", "📋 View Inventory", "📜 Transfer Log"])
...
# Try session cookie auto-login (relies on _read_cookie from your auth block)
if not st.session_state.authenticated and not st.session_state.get("just_logged_out"):
    payload = _read_cookie()
    if payload:
        st.session_state.authenticated = True
