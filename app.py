...
# =============================================================================
# OAUTH (Drive uploads use user OAuth; Sheets uses Service Account)
# =============================================================================
...
def _load_oauth_client_config() -> dict:
    """
    Load Google OAuth client config for user sign-in (Drive uploads).

    Priority:
      1) st.secrets["gmail_oauth"] -> keys: json OR json_b64, optional: mode
      2) env GOOGLE_OAUTH_CLIENT_JSON or GOOGLE_OAUTH_CLIENT_B64
      3) local file ./credentials.json

    Returns a dict:
      {"mode": "config", "client": {...}, "auth_mode": "local_server"|"console"}
      or {"mode": "file", "path": "credentials.json", "auth_mode": "..."}
      or {"mode": None, "auth_mode": "..."} if not found.
    """
...
COOKIE_SAMESITE = st.secrets.get("auth", {}).get("cookie_samesite", "Lax")

SHEET_URL_DEFAULT = st.secrets.get("sheets", {}).get(
    "url",
...
def _sign(raw: bytes, key: str) -> str:
...
def _issue_session_cookie(username: str, role: str):
    ...
    key = st.secrets.get("auth", {}).get("cookie_key", "PLEASE_SET_auth.cookie_key_IN_SECRETS")
    token = base64.urlsafe_b64encode(raw).decode() + "." + _sign(raw, key)
    COOKIE_MGR.set(
        COOKIE_NAME, token,
...
def _read_cookie():
    ...
    try:
        ...
        for k in _cookie_keys():
            if hmac.compare_digest(sig, _sign(raw, k)):
...
# first run: ensure JS cookie bridge is ready
...
def _font_candidates():
    ...
    cands += ["company_font.ttf","company_font.otf","ACBrandFont.ttf","ACBrandFont.otf",
...
def _apply_brand_font():
    ...
    for p in _font_candidates():
        if os.path.exists(p):
            _inject_font_css(p, family=fam); return
...
# =============================================================================
# GOOGLE SHEETS (Service Account) & DRIVE HELPERS
# =============================================================================
...
def _load_sa_info() -> dict:
    ...
    if isinstance(raw, dict):
        ...
        try:
            ...
            st.error("❌ gcp_service_account in secrets is not valid JSON string.")
            sa = {}
    ...
    if "private_key" not in sa:
        raise RuntimeError(
            ...
            "Add it to secrets as [gcp_service_account] or set GOOGLE_SERVICE_ACCOUNT_JSON."
        )
    return sa
...
def upload_pdf_and_link(uploaded_file, *, prefix: str) -> tuple[str, str]:
    """Upload PDF to user's Drive (OAuth). Returns (webViewLink, file_id)."""
    ...
    drive = get_gmail_drive()  # user OAuth
...
def _read_ws_as_dataframe(ws, expected_cols: list[str]) -> tuple[pd.DataFrame, int, int]:
...
def _read_worksheet_cached(ws_title: str) -> pd.DataFrame:
    if ws_title == EMPLOYEE_WS:
        ...
        st.session_state.emp_debug = {"title": ws.title, "gid": ws.id, "header_row": header_idx + 1,
                                      "overlap": overlap, "rows": len(df)}
        return df
...
def register_device_tab():
    ...
    with st.form("register_device", clear_on_submit=True):
        ...
        with r1c2:
            ...
            if assigned_choice == "Type a new name…": assigned_to = st.text_input("Assignee name")
            elif assigned_choice == UNASSIGNED_LABEL: assigned_to = UNASSIGNED_LABEL
            else: assigned_to = assigned_choice
    ...
    if not s_norm:
        st.error("Serial Number cannot be blank after normalization."); return
    ...
    if not inv.empty:
        ...
        near = inv[near_mask]
        if not near.empty:
            st.warning("Near-duplicate serials: " + ", ".join(near["Serial Number"].astype(str).unique().tolist()))
    ...
    if not is_admin and pdf_file is None:
        st.error("Approval PDF is required for submission."); return
...
def transfer_tab():
    ...
    if inventory_df.empty:
        st.warning("Inventory is empty."); return
...
def employee_register_tab():
    ...
    if not emp_name.strip():
        st.error("New Employeer is required."); return
            ...
            with st.expander(f"{row['Device Type']} — SN {row['Serial Number']} (by {row['Submitted by']})", expanded=False):
                ...
                with c2:
                    ...
                    if r_col.button("Reject", key=f"reject_dev_{i}"):
                        _mark_decision(PENDING_DEVICE_WS, row, status="Rejected"); st.info("❌ Request rejected.")
            ...
            with st.expander(f"SN {row['Serial Number']}: {row['From owner']} → {row['To owner']} (by {row['Submitted by']})", expanded=False):
                ...
                with c2:
                    ...
                    if r_col.button("Reject", key=f"reject_tr_{i}"):
                        _mark_decision(PENDING_TRANSFER_WS, row, status="Rejected"); st.info("❌ Request rejected.")
...
def export_tab():
    ...
    if not approved_dev.empty:
        approved_dev = approved_dev[approved_dev.get("Approval Status", "").astype(str) == "Approved"]
    if not approved_tr.empty:
        approved_tr  = approved_tr[approved_tr.get("Approval Status", "").astype(str) == "Approved"]
...
def run_app():
    ...
    if st.session_state.role == "Admin":
        tabs = st.tabs([
            "🧑‍💼 Employee Register","📇 View Employees","📝 Register Device","📋 View Inventory",
            "🔁 Transfer Device","📜 Transfer Log","✅ Approvals","⬇️ Export",
        ])
...
# =============================================================================
# GOOGLE SHEETS & DRIVE (service account for Sheets; user OAuth for Drive uploads)
# =============================================================================
SCOPES = [
    "https://www.googleapis.com/auth/spreadsheets",
    "https://www.googleapis.com/auth/drive",
...
def _load_sa_info() -> dict:
    ...
    if isinstance(raw, dict):
        ...
        try:
            ...
            st.error("❌ gcp_service_account is not valid JSON string.")
            sa = {}
